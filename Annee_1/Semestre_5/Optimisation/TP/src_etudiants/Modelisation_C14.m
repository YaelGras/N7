...
for i = 1:m
   for j = 1:n
      ...
      % TO DO %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
      f_plot(i,j) = 1/2 * res_plot' * res_plot; %1/2 de la norme au carré
   end
end
...
%          real(n)
%
    Matrice = beta(1) * exp(-beta(2) * donnees(: , 1)) - donnees(:,2);
    res = Matrice;
...
%
    Matrice = zeros(7, 2);
    Matrice (:, 1) = exp(-beta(2) * donnees(: , 1)); %Dérivée selon A_0 (beta(1))
    Matrice (:, 2) = - beta(1) .* donnees(:, 1) .* exp(-beta(2) * donnees(: , 1)); %Derivée selon lambda
    J_res = Matrice;
...
%
    ...
    for i = 1:7
        Matrice = zeros (2,2);
        Matrice(1,1) = 0;
        Matrice(1,2) = -donnees(i,1) * exp(-beta(2) * donnees(i , 1));
        Matrice(2,1) = -donnees(i,1) * exp(-beta(2) * donnees(i , 1));
        Matrice(2,2) = beta(1)* donnees(i,1)^2 * exp(-beta(2) * donnees(i , 1));
        Hes_r_r = Hes_r_r + Matrice*res(i);
    end

    H_f = Hes_r_r + J_res' * J_res;
