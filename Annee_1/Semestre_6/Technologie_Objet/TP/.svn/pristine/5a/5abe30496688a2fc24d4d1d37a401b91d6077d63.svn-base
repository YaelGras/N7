...
public class PointDeVente {
	...
	public int getPrix(Carburant carburant, LocalDateTime date) {
			...
			if(this.prix.containsKey(carburant)) {
				try {
				NavigableMap<LocalDateTime, Integer>  prix_carburant = prix.get(carburant);
				return prix_carburant.get(prix_carburant.floorKey(date));
				} catch(NullPointerException e) {
					return 0; // La date est avant le premier changement de prix
				}
			} else {
				return 0; // Le carburant n'est pas vendu
			}
