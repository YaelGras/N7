...
int main(int argc, char *argv[]) {
    int ret_add, fic_in, fic_out, pipe_pr[2];
    pid_t pidFils;
    ...
    fic_in = -10;
    fic_out = -10;
    printf("Bienvenue dans le minishell v1 développé par Yael Gras :");
    do {   
        ...
        if(commande == NULL) {
            ...
            if(commande->in != NULL) {
                fic_in = open(commande->in, O_RDONLY );
                if(fic_in < 0) {
                    ...
                    switch(pidFils=fork()) 
                    {
                        ...
                        case 0 : //FILS
                            ...
                            if (commande->seq[i+1]!= NULL){
                                dup2(pipe_pr[1], 1);
                            } else if (commande->out != NULL){
                                fic_out = open(commande->out, O_WRONLY | O_CREAT | O_TRUNC, S_IRWXU);
                                if(fic_out < 0) {
