...
void ajouter(char *dep) { // traite la demande de connexion du pseudo référencé par dep
	for(int i = 0; i < MAXPARTICIPANTS; i++) {
		if(!participants[i].actif) {
			...
			//Ouverture d'un pipe de lecture
			mkfifo(strcat(dep, "c2s"), S_IRUSR|S_IWUSR);
			participants.[i].out = open(strcat(dep, "c2s"), O_WRONLY);
			//Ouverture d'un pipe en écriture
			mkfifo(strcat(dep, "s2c"), S_IRUSR|S_IWUSR);
			participants.[i].out = open(strcat(dep, "s2c"), O_WRONLY);
		}
	}
}
